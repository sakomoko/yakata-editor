...
Canvas上でのマウス操作 → editor/ のイベントハンドラ → state更新 → commitChange() → pushUndo() + render() + callbacks.onAutoSave() というデータフローで動作する。エディタはストレージに直接依存せず、App.tsxが `project-store.ts` を通じてアクティブプロジェクトのキーに保存する。
...
- **project-store.ts** — マルチプロジェクト対応のlocalStorageストレージ層。プロジェクトindex・データ・タブ状態のCRUD、旧形式からの自動マイグレーション
