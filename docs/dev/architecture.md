...
- **adjacency.ts** — 隣接部屋の壁オブジェクト自動同期。壁面の対面取得（`getOppositeSide`）、隣接部屋検索（`findAdjacentRoomsOnWall`）、offset座標変換（`convertOffset`）、ペア開口の同期（`syncPairedOpening`）・削除（`removePairedOpening`）・全再構築（`syncAllPairedOpenings`）、自動生成開口の判定（`isPairedAutoOpening`）
