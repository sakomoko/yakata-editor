import type { Room, Arrow, FreeText, FreeStroke, WallSide } from '../types.ts';
...
export function copySelection(ec: EditorContext): void {
  ...
  const clonedArrows: Arrow[] = structuredClone(arrows);

  for (const r of clonedRooms) {
    cleanRoomForClipboard(r);
  }

  const bb = computeBoundingBox(clonedRooms, clonedFreeTexts, clonedFreeStrokes, clonedArrows);
...
export function pasteClipboard(
  ...
  const clonedArrows: Arrow[] = structuredClone(clipboard.arrows);

  // Apply mirror before offset (mirror around clipboard center)
  if (mirror === 'horizontal') {
    const centerGx = clipboard.originGx + clipboard.bbW / 2;
    applyMirrorHorizontal(clonedRooms, clonedFreeTexts, clonedFreeStrokes, clonedArrows, centerGx);
  } else if (mirror === 'vertical') {
    const centerGy = clipboard.originGy + clipboard.bbH / 2;
    applyMirrorVertical(clonedRooms, clonedFreeTexts, clonedFreeStrokes, clonedArrows, centerGy);
  }

  regenerateIds(clonedRooms, clonedFreeTexts, clonedFreeStrokes, clonedArrows);
  ...
  applyOffset(clonedRooms, clonedFreeTexts, clonedFreeStrokes, clonedArrows, dx, dy);
  ...
  commitChange(ec, () => {
    ...
    state.arrows.push(...clonedArrows);
    ...
    for (const a of clonedArrows) state.selection.add(a.id);
  });
}
